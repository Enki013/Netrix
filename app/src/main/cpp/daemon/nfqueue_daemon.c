...
static void parse_settings_from_command(const char* cmd, DpiBypassSettings* settings);
...
static int setup_server_socket(void) {
    ...
    // Restrict socket to root clients. The Android app talks to it through
    // Magisk su + nc, because SELinux blocks normal app/shell UIDs here.
...
static void parse_settings_from_command(const char* cmd, DpiBypassSettings* settings) {
    ...
    if ((ptr = strstr(cmd, "\"first_packet_size\":")) != NULL) {
        int value = atoi(ptr + 20);
        if (value > 0 && value <= 64) settings->first_packet_size = value;
    }
    if ((ptr = strstr(cmd, "\"split_delay\":")) != NULL) {
        int value = atoi(ptr + 14);
        if (value >= 0 && value <= 10000) settings->split_delay_ms = value;
    }
    if ((ptr = strstr(cmd, "\"split_count\":")) != NULL) {
        int value = atoi(ptr + 14);
        if (value > 0 && value <= 32) settings->split_count = value;
    }
    ...
    if (strstr(cmd, "\"mix_host_case\":true")) settings->mix_host_case = true;
...
static int parse_and_execute_command(const char* cmd, char* response, size_t resp_size) {
    ...
    if (strstr(cmd, "\"cmd\":\"start\"") || strstr(cmd, "\"cmd\": \"start\"")) {
        ...
        // Apply settings bundled with the START command before iptables/NFQUEUE
        // starts, so first-run root mode matches the UI configuration.
        DpiBypassSettings settings;
        DpiBypassSettings* current = dpi_bypass_get_settings();
        memcpy(&settings, current, sizeof(settings));
        parse_settings_from_command(cmd, &settings);
        ...
        // Parse settings from JSON-ish command payload
        DpiBypassSettings settings;
        DpiBypassSettings* current = dpi_bypass_get_settings();
        memcpy(&settings, current, sizeof(settings));
        parse_settings_from_command(cmd, &settings);
